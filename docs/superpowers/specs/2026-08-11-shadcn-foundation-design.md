...
| dev dep `@types/node`, and `"types": ["vite/client", "node"]` in `tsconfig.json` | the seam test — see *Reading files from a test* |
...
### Reading files from a test

Found while implementing, not anticipated at design time.

The seam test cannot render anything — jsdom does not run Tailwind, so the token has no computed
value. It reads the files instead, and **the obvious way is silently broken**: Vite's `?raw` works
for `index.html` and `tauri.conf.json`, but the Tailwind Vite plugin intercepts `.css` requests and
returns an **empty string**. The first version of this test compared `""` against `""` and would
have passed while proving nothing — the exact failure this repo has been bitten by before.

So CSS is read with `node:fs`, which needs `@types/node`. That dependency was initially rejected on
the grounds that `vite.config.ts` carries `// @ts-expect-error process is a nodejs global` and the
project therefore "deliberately does without" Node typings. That reading was wrong: the comment is
generated by `create-tauri-app`. Adding `@types/node` does not invalidate it either, because
`vite.config.ts` is covered by `tsconfig.node.json`, not the `include: ["src"]` project.

The zustand filter store is a module-level singleton, so `FilterBar.test.tsx` resets it in
`beforeEach`. Without that, a test that selects a filter changes what every later test renders.

### `toggleVariants` is not exported

shadcn ships it as a public export. Nothing here consumes it, and exporting a non-component
alongside a component trips `react-refresh/only-export-components`. It stays module-local until
something needs it.
